...
ID3v2_tag* load_tag(const char* file_name)
{
    ...
    // get header size
    ID3v2_header *tag_header = get_tag_header_with_file(file);
    if(tag_header == NULL) {
        return NULL;
    }
    ...
    tag = load_tag_with_file(file, tag_header, header_size);
    free(buffer);
...
// header is optional
ID3v2_tag* load_tag_with_file(FILE *file, ID3v2_header *header, int length)
{
    // Declaration
    ID3v2_frame *frame;
    int offset = 0;
